...
def init_args(args):
    ...
    args.modelSavePath    = os.path.join(args.save_path, 'model')
    args.scoreSavePath    = os.path.join(args.save_path, 'score.txt')
    args.trialPathAVA     = os.path.join(args.AVA_data_path, 'csv')
    args.audioOrigPathAVA = os.path.join(args.AVA_data_path, 'orig_audios')
    args.visualOrigPathAVA= os.path.join(args.AVA_data_path, 'orig_videos')
    args.audioPathAVA     = os.path.join(args.AVA_data_path, 'clips_audios')
    args.visualPathAVA    = os.path.join(args.AVA_data_path, 'clips_videos')
    ...
    if args.evalDataType == 'val':
        ...
        args.evalCsvSave  = os.path.join(args.save_path,     'val_res.csv') 
    else:
        ...
        args.evalCsvSave  = os.path.join(args.save_path,     'test_res.csv')

    os.makedirs(args.modelSavePath, exist_ok = True)
    os.makedirs(args.AVA_data_path, exist_ok = True)
...
def download_csv(args): 
    ...
    cmd = "gdown --id %s -O %s"%(Link, args.AVA_data_path + '/csv.tar.gz')
    subprocess.call(cmd, shell=True, stdout=None)
    cmd = "tar -xzvf %s -C %s"%(args.AVA_data_path + '/csv.tar.gz', args.AVA_data_path)
    subprocess.call(cmd, shell=True, stdout=None)
    os.remove(args.AVA_data_path + '/csv.tar.gz')
